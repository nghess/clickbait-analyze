...
class ClusterlessDecoder(_DecoderBase):
    ...
    def fit(
        ...
        # fitting the multiunits to the decoder in batches
        # Batch size is set to 10 million
        batch_size = 10_000_000
        print(f'number of batches: {position.shape[0] // batch_size + 1}')
        for i in range(0, position.shape[0], batch_size):
            #logging.info(f"Fitting decoder for batch {i // batch_size + 1}")
            self.fit_multiunits(position[i:i + batch_size], multiunits[i:i + batch_size])
